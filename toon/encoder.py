...
class Encoder:
    ...
    def _encode_array(self, key: Optional[str], value: JsonArray, writer: LineWriter, depth: Depth):
        if not value:
    ...
    def _encode_inline_primitive_array(self, prefix: Optional[str], values: JsonArray, writer: LineWriter, depth: Depth):
    ...
    def _encode_array_of_objects_as_tabular(self, prefix: Optional[str], rows: JsonArray, header: list[str], writer: LineWriter, depth: Depth):
        header_str = format_header(len(rows), key=prefix, fields=header, delimiter=self.delimiter, length_marker=self.length_marker)
    ...
    def _encode_mixed_array_as_list_items(self, prefix: Optional[str], items: JsonArray, writer: LineWriter, depth: Depth):
        header = format_header(len(items), key=prefix, delimiter=self.delimiter, length_marker=self.length_marker)
        writer.push(depth, header)

        for item in items:
            if is_json_primitive(item):
                writer.push(depth + 1, f'{LIST_ITEM_PREFIX}{encode_primitive(item, self.delimiter)}')
    ...
    def _encode_object_as_list_item(self, obj: JsonObject, writer: LineWriter, depth: Depth):
        ...
        if is_json_primitive(first_value):
            writer.push(depth, f'{LIST_ITEM_PREFIX}{encoded_key}: {encode_primitive(first_value, self.delimiter)}')
