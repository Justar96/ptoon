...
This module handles conversion of Python types to JSON-compatible values
before TOON encoding. Normalizes special types like datetime, set, and
handles edge cases like non-finite floats and large integers.

Normalization Rules:
    - datetime → ISO 8601 string
    - set → sorted list
    - Large integers (>2^53-1) → string
    - Non-finite floats (inf, -inf, NaN) → null
    - Negative zero → 0
    - Mapping types → dict with string keys
...
def normalize_value(value: Any) -> JsonValue:
    ...
    if isinstance(value, Mapping):
        ...
        try:
            ...
            raise ValueError(
                f"Failed to convert mapping to dict: {e}. Check that all keys can be converted to strings."
            ) from e
